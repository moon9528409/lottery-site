...
<body>
  ...
  <script>
    ...
    function addRow(item="", winner="", note="") {
      const row = document.createElement("tr");
      row.innerHTML = `
        <td contenteditable="${editMode}">${item}</td>
        <td contenteditable="${editMode}">${winner}</td>
        <td contenteditable="${editMode}">${note}</td>
        <td class="editCol ${editMode ? "" : "hidden"}">
    ...
    toggleModeBtn.addEventListener("click", () => {
      ...
      editMode = !editMode;
      toggleModeBtn.textContent = "目前模式：" + (editMode ? "編輯" : "觀看");
      [addRowBtn, saveBtn, loadBtn, importBtn, exportBtn].forEach(btn => btn.classList.toggle("hidden", !editMode));
      document.querySelectorAll(".editCol").forEach(el => el.classList.toggle("hidden", !editMode));
      tableBody.querySelectorAll("tr").forEach(row => {
        row.querySelectorAll("td").forEach((td, i) => {
          if (i < 3) td.contentEditable = editMode;
        });
        const actionCell = row.querySelector("td:last-child");
        if (actionCell) {
          actionCell.classList.toggle("hidden", !editMode);
          if (editMode) {
            actionCell.innerHTML = '<button class="danger">刪除</button>';
